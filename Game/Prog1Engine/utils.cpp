...
#pragma region CollisionFunctionality
	...
	float GetDistance(const Point2f& coords1, const Point2f& coords2)
	{
		float xCoords{ (coords2.x - coords1.x) },
			yCoords{ (coords2.y - coords1.y) };
	...
	bool IsPointInCircle(const Point2f& circleCenter, float radius, const Point2f& pointLocation)
	{
		if (abs(pointLocation.x - circleCenter.x) <= radius && abs(pointLocation.y - circleCenter.y) <= radius)
			return true;
		else
			return false;
	}

	bool IsPointInCircle(const Circlef& circle, const Point2f& pointLocation)
	{
		return IsPointInCircle(circle.center, circle.radius, pointLocation);
	}

	bool IsPointInRect(const Rectf& square, const Point2f& pointLocation)
	{
		if (pointLocation.x >= square.left && pointLocation.x <= square.left + square.width
			&& pointLocation.y >= square.top && pointLocation.y <= square.top + square.height)
			return true;
		else
			return false;
	}
	...
	bool IsPointInRect(const Point2f& topLeftCorner, float width, float height, const Point2f& pointLocation)
	{
		return IsPointInRect(Rectf{ topLeftCorner.x, topLeftCorner.y, width, height }, pointLocation);
	}
	...
	bool IsOverlapping(const Rectf& rectangle1, const Rectf& rectangle2)
	{
		if (rectangle1.left > rectangle2.left + rectangle2.width || rectangle2.left > rectangle1.left + rectangle1.width)
			return false;
		if (rectangle1.top > rectangle2.top + rectangle2.height || rectangle2.top > rectangle1.top + rectangle1.height)
			return false;
		return true;
	}
	...
	bool IsOverlapping(const Circlef& first, const Circlef& second)
	{
		...
		return (first.radius + second.radius) * (first.radius + second.radius) <=
